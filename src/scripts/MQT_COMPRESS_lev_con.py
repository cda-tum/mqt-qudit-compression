...
from running_utils import run_clustering_con

paths = load_config("config.json")

path = paths["selected_data"]
path_save = paths["connected_levels_solutions"]
csv_path = paths["connected_levels_solutions_csv"]
...
for ix, _f in enumerate(fileslist):
    ...
    for iteration in range(1):
        for QUDIT_SIZE in [2, 3, 4, 5]:
            ...
            inputf = (
                path_save + "/graphs/" + filename + "_unlab_lev_con" + str(iteration) + "_" + str(QUDIT_SIZE) + ".txt"
            )
            outputf = (
                path_save
                + "/result_from_graph/"
            ...
            (
                ...
                new_col_by_node,
            ) = run_clustering_con(G, inputf, outputf, K, 2)
            ...
            if False in valid:
                (
                    ...
                    saved_weight_by_color,
                    new_col_by_node,
                ) = run_clustering_con(G, inputf, outputf, K, 1)
                color_by_node = new_col_by_node
                valid = []
                for color in nodes_by_color.values():
                    H = G.subgraph(color)
